...
namespace dsr_control{
    ...
    */
    ...
    void DRHWInterface::thread_keepalive()
    {   
        ROS_INFO("[KeepAlive] thread_keepalive() start!");

        ros::Rate rate(1.0/60.0);  // 약 1min check 
        ros::Duration timeout_interval(240.0);  // 4분 이상 trajectory 미수신 시 dummy 전송
        ...
        while (ros::ok())
        {
            ...
            double time_since_last_trajectory = 0.0;
            {
                std::lock_guard<std::mutex> lock(keepalive_mutex_);
                time_since_last_trajectory = (now - last_trajectory_time_).toSec();


                // // 현재까지 미수신된 시간 출력
                // ROS_INFO_STREAM_THROTTLE(5.0, "[KeepAlive] time elapsed after final trajectory: " 
                //                             << std::fixed << std::setprecision(2)
                //                             << time_since_last_trajectory << "s");
        ...
        m_server_joint_trajectory(nh, "dsr_joint_trajectory_controller/follow_joint_trajectory")
    {
        ...
        */
        //example_subscriber_node = private_nh_.subscribe("/topic", 10, &DRHWInterface::Impedancecontrol_cb, this);
        trajectory_subscriber_node = private_nh_.subscribe("/cartesian_trajectory", 100, &DRHWInterface::trajectory_cb,this);
        ...
        //push_pull_rt = private_nh_.subscribe("/trajectory", 20, &DRHWInterface::Impedancecontrol_cb, this);
    ...
    bool DRHWInterface::init()
    {
        ...
        m_th_keepalive = boost::thread(boost::bind(&DRHWInterface::thread_keepalive, this));


        ROS_INFO("host %s, port=%d bCommand: %d, mode: %s\n", host.c_str(), nServerPort, bCommand_, mode.c_str());
        
        if(Drfl.open_connection(host))
        {   
            cout << "connected" << endl;
            //--- connect Emulator ? ------------------------------
            ...
            if(mode == "real") {
                eTargetSystem = ROBOT_SYSTEM_REAL;
            }
            else {
                eTargetSystem = ROBOT_SYSTEM_VIRTUAL;
            }
            ...
            if(Drfl.connect_rt_control(host))
            {
                ...
                Drfl.add_tool("gripper", fweight, fCog, finertia);
                Drfl.set_tool("gripper");
    ...
    bool DRHWInterface::connect_rt()
    {
        if(Drfl.connect_rt_control("192.168.137.100"))
        {
            cout << "RT connection completed" << endl;
        }
        string version = "v1.0";
        ...
        cout << "RT setting completed" << endl;
        Drfl.start_rt_control();
        cout << "RT control start" << endl;
        initialized_ = true;
        Drfl.set_robot_mode(ROBOT_MODE_AUTONOMOUS); 
        Drfl.set_safety_mode(SAFETY_MODE_AUTONOMOUS, SAFETY_MODE_EVENT_MOVE);
        Drfl.change_collision_sensitivity(1);
    ...
    // void initBag() {
    // try {
    //         bag.open("/catkin_ws/data/trajecotry_1.bag", rosbag::bagmode::Write);
    ...
    //     try {
    //         bag.close();
    //         ROS_INFO("Bag file closed successfully.");
    //     } catch (const std::exception &e) {
    //         ROS_ERROR("Failed to close bag file: %s", e.what());
    //     }
    // }


    void DRHWInterface::trajectory_cb(moveit_msgs::CartesianTrajectory input_msg) {
        
        ROS_INFO("Trajectory callback");
        {
            std::lock_guard<std::mutex> lock(keepalive_mutex_);
            last_trajectory_time_ = ros::Time::now();
        }
        // initBag();
        // try {
        //     bag.write("/trajectory_topic", ros::Time::now(), input_msg);
        //     ROS_INFO("Message saved to bag file.");
        // } catch (const std::exception &e) {
        //     ROS_ERROR("Failed to write to bag file: %s", e.what());
        // }
        // ROS_INFO_STREAM("Full message content original : " << input_msg);
        ...
        if(!controlState){
            ...
            moveit_msgs::CartesianTrajectory converted_msg = trajectoryConverter(input_msg);
            ROS_INFO("Trajectory converted and unwrapped");

            std::string tracked_frame_lower = converted_msg.tracked_frame;
            // ROS_INFO_STREAM("Full message content converted : " << converted_msg);
            std::transform(tracked_frame_lower.begin(), tracked_frame_lower.end(), tracked_frame_lower.begin(), ::tolower);
          
            // check plz 
            if (tracked_frame_lower == "position_goal") {
                PositionGoalcontrol(converted_msg);
            }
            else if (tracked_frame_lower == "impedance_goal") {
                ImpedanceGoalcontrol(input_msg);
            }
            else if (tracked_frame_lower == "position_path") {
                PositionPathcontrol(converted_msg);
            }
            else if (tracked_frame_lower == "position_jpath") {
                PositionJPathcontrol(converted_msg);
            }
            else if (tracked_frame_lower == "impedance_path") {
                ImpedancePathcontrol(converted_msg);
            }
            else if (tracked_frame_lower == "home") {
                // float home_j[NUMBER_OF_JOINT] = {0.0, 8.32, 49.12, -0.1, 122.55, 0}; // Above 
                // float home_j[NUMBER_OF_JOINT] = {3.0, 2, 89.87, -0.1, 50, -30}; // for DBIC 
                float home_j[NUMBER_OF_JOINT] = {2.98, 9.13, 79.35, 0.03, 92.53, 10}; // for PBIC
                // float home_j[NUMBER_OF_JOINT] = {0.0, -11.63, 84.76, -0.1, 90.56, 0};
                // float home_j[NUMBER_OF_JOINT] = {0.0, -11.63, 84.76, -0.1, 130.56, 0};
                Drfl.movej(home_j, 60, 30);
                return;
            }

            else if (tracked_frame_lower == "home_s") {
                // float home_j[NUMBER_OF_JOINT] = {0.0, 8.32, 49.12, -0.1, 122.55, 0}; // Above 
                // float home_j[NUMBER_OF_JOINT] = {3.0, 2, 89.87, -0.1, 50, -30}; // for DBIC 
                float home_j[NUMBER_OF_JOINT] = {2.98, 9.13, 79.35, 0.03, 92.53, 10}; // for PBIC
                // float home_j[NUMBER_OF_JOINT] = {0.0, -11.63, 84.76, -0.1, 90.56, 0};
            ...
            ROS_INFO_STREAM("Full message content: " << converted_msg);

            ROS_INFO("Control completed cleanly");
            // closeBag();
        }
    }
    
    double DRHWInterface::unwrapAngle(double previous_angle, double current_angle) {
    ...
    moveit_msgs::CartesianTrajectory DRHWInterface::trajectoryConverter(const moveit_msgs::CartesianTrajectory &input_msg) {
        ROS_INFO("Received trajectory with %lu points", input_msg.points.size());

        moveit_msgs::CartesianTrajectory new_msg = input_msg; // 원본 메시지를 복사
        double previous_roll = 0.0, previous_pitch = 0.0, previous_yaw = 0.0; // RZYZ angles

        for (size_t i = 0; i < new_msg.points.size(); i++) {
            // Extract quaternion from the pose
            double qx = input_msg.points[i].point.pose.orientation.x;
            double qy = input_msg.points[i].point.pose.orientation.y;
            double qz = input_msg.points[i].point.pose.orientation.z;
            double qw = input_msg.points[i].point.pose.orientation.w;

            double yaw = atan2(2 * (qw * qz + qx * qy), 1 - 2 * (qy * qy + qz * qz));
            double pitch = asin(std::min(1.0, std::max(-1.0, 2 * (qw * qy - qz * qx))));
            ...
            // yaw = -yaw;  // Reverse the direction if needed

                 
            double roll_deg = rad_to_deg(roll);
            double pitch_deg = rad_to_deg(pitch);
            double yaw_deg = rad_to_deg(yaw);

            // Unwrap angles for continuity
            roll_deg = unwrapAngle(previous_roll, roll_deg);
            pitch_deg = unwrapAngle(previous_pitch, pitch_deg);
            yaw_deg = unwrapAngle(previous_yaw, yaw_deg);


            ROS_INFO("Point %lu: Roll = %f deg, Pitch = %f deg, Yaw = %f deg",
                 i, roll_deg, pitch_deg, yaw_deg);

        
            previous_roll = roll_deg;
            previous_pitch = pitch_deg;
            previous_yaw = yaw_deg;

       
            // Store RZYZ angles in the orientation fields
            new_msg.points[i].point.pose.orientation.x = roll_deg;
            new_msg.points[i].point.pose.orientation.y = pitch_deg;
            new_msg.points[i].point.pose.orientation.z = yaw_deg;
            new_msg.points[i].point.pose.orientation.w = 0.0; // Set w as needed
        }

        return new_msg; // Return the new message
    }

    double DRHWInterface::rad_to_deg(double radian) {
        return radian * (180.0 / M_PI);
    }

    void DRHWInterface::interpolation(SKKU::Total_trajectory*& total_trajectory_) {
        std::vector<float> data = {-34.5, 559.00, 400, 93.42, -180, 3.42};
        for (int i = 0; i < 2000; i++) {
            
            total_trajectory_ -> pos_d.push_back(data);
            
        }
    }

    /*
    void DRHWInterface::cartesianTrajectoryCallback(const moveit_msgs::CartesianTrajectory::ConstPtr& msg) {
        const std::vector<geometry_msgs::Pose>& poses = msg -> points;

        // Extract position, velocity, acceleration, orientation, and time data from each point
        std::vector<std::array<float, 6>> pos_d{};
        std::vector<std::array<float, 6>> vel_d{};
        std::vector<std::array<float, 6>> acc_d{};
        std::vector<float> time_d{};

        for (const auto& point : poses) {
            std::array<float, 6> pos = { point.position.x, point.position.y, point.position.z, 0.0, 0.0, 0.0 };
            std::array<float, 6> vel = { point.velocity.x, point.velocity.y, point.velocity.z, 0.0, 0.0, 0.0 };
            std::array<float, 6> acc = { point.acceleration.x, point.acceleration.y, point.acceleration.z, 0.0, 0.0, 0.0 };
            ...
            tf2::fromMsg(point.velocity, vel_quat);
            double vel_roll, vel_pitch, vel_yaw;
            tf2::Matrix3x3(vel_quat).getRPY(vel_roll, vel_pitch, vel_yaw);

            vel[3] = vel_roll;
            vel[4] = vel_pitch;
            vel[5] = vel_yaw;

            tf2::Quaternion acc_quat;
            tf2::fromMsg(point.acceleration, acc_quat);
            double acc_roll, acc_pitch, acc_yaw;
            tf2::Matrix3x3(acc_quat).getRPY(acc_roll, acc_pitch, acc_yaw);
            ...
            time_d.push_back(point.time_from_start.toSec());  // Assuming time_from_start is in seconds
        }

        Total_trajectory trajectory_data(pos_d, vel_d, acc_d, time_d);
        stored_trajectory_data = Total_trajectory(pos_d, vel_d, acc_d, time_d);
    }
    ...
    float DRHWInterface::linearInterpolation(float x, float x0, float y0, float x1, float y1) {
        return y0 + (x - x0) * (y1 - y0) / (x1 - x0);
    }

    Total_trajectory DRHWInterface::interpolateTrajectory(Trajectory& original_data, float new_sampling_rate) {
        int num_samples = static_cast<int>(original_data.time_d.back() * new_sampling_rate) + 1;

        std::vector<float> new_time_data(num_samples);
        for (int i = 0; i < num_samples; ++i) {
            new_time_data[i] = static_cast<float>(i) / new_sampling_rate;
        }


        Total_trajectory interpolated_data;

        for (int joint = 0; joint < 6; ++joint) {
            int prev_idx = 0;
            for (int i = 0; i < new_time_data.size(); ++i) {
                float new_time = new_time_data[i];
                while (prev_idx < original_data.time_d.size() - 1 && new_time > original_data.time_d[prev_idx + 1]) {
                    prev_idx++;
                }
                if (prev_idx < original_data.time_d.size() - 1) {
                    float x0 = original_data.time_d[prev_idx];
                    float y0 = original_data.pos_d[prev_idx][joint];
                    float x1 = original_data.time_d[prev_idx + 1];
                    float y1 = original_data.pos_d[prev_idx + 1][joint];
                    float interpolated_value = linearInterpolation(new_time, x0, y0, x1, y1);
                    interpolated_data.pos_d[i * 6 + joint] = interpolated_value; // position

                    float y0_vel = original_data.vel_d[prev_idx][joint];
                    float y1_vel = original_data.vel_d[prev_idx + 1][joint];
                    float interpolated_value_vel = linearInterpolation(new_time, x0, y0_vel, x1, y1_vel);
                    interpolated_data.vel_d[i * 6 + joint] = interpolated_value_vel; // velocity

                    float y0_acc = original_data.acc_d[prev_idx][joint];
                    float y1_acc = original_data.acc_d[prev_idx + 1][joint];
                    float interpolated_value_acc = linearInterpolation(new_time, x0, y0_acc, x1, y1_acc);
                    interpolated_data.acc_d[i * 6 + joint] = interpolated_value_acc; // acceleration
                }
            }
        }
    ...
    */
    ...
    void DRHWInterface::ImpedanceGoalcontrol(moveit_msgs::CartesianTrajectory msg) {
        // /*ration while another control or read operation is running.");
        // }
        // if (!initialized_) {
        //     ROS_ERROR("Error: Call to control before initialization!");
        //     return;
        // }
        // */
        ...
        if (!l.owns_lock()) {
            throw std::runtime_error("Error : Cannot perform this operation");
        }

        static SKKU::ImpedanceControlLoop impedance_loop(msg, loop_time, realtime_config, Drfl);
        ROS_INFO_STREAM("Full message content FIRST : " << msg);
        impedance_loop(msg);

        return;
    }

    void DRHWInterface::PositionGoalcontrol(moveit_msgs::CartesianTrajectory msg) {
        // /*ration while another control or read operation is running.");
        // }
        // if (!initialized_) {
        //     ROS_ERROR("Error: Call to control before initialization!");
        ...
        static SKKU::PositionControlLoop position_loop(msg, loop_time, realtime_config, Drfl);
        position_loop(msg);

        return;
    }

    void DRHWInterface::ImpedancePathcontrol(moveit_msgs::CartesianTrajectory msg) {
        // /*ration while another control or read operation is running.");
        // }
        // if (!initialized_) {
        //     ROS_ERROR("Error: Call to control before initialization!");
        ...
        impedance_loop.operator_path(msg);

        return;
    }

    void DRHWInterface::PositionPathcontrol(moveit_msgs::CartesianTrajectory msg) {
        // /*ration while another control or read operation is running.");
        // }
        // if (!initialized_) {
        //     ROS_ERROR("Error: Call to control before initialization!");
        ...
        static SKKU::PositionControlLoop position_loop(msg, loop_time, realtime_config, Drfl);
        position_loop.operator_path(msg);

        return;
    }

    void DRHWInterface::PositionJPathcontrol(moveit_msgs::CartesianTrajectory msg) {
        // /*ration while another control or read operation is running.");
        // }
        // if (!initialized_) {
        //     ROS_ERROR("Error: Call to control before initialization!");
        ...
        static SKKU::PositionControlLoop position_loop(msg, loop_time, realtime_config, Drfl);
        position_loop.operator_jpath(msg);
    ...
    void DRHWInterface::trajCallback(const moveit_msgs::CartesianTrajectoryConstPtr& msg)
    {
        // Process the received Cartesian trajectory data here
        // Example: Print the received position and time_from_start
        ROS_INFO("Received Cartesian Trajectory!!");
